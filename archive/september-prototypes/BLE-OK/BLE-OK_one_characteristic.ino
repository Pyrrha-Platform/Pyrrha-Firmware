...
// With this kind of classes y can identify if the client app is writing a characteristic and we can take actions. In this case we get the Date and Time from the mobile app
...
void loop() {
    ...
    // With this delay we can control how many times we are going to read the sensor in one minute (the idea is to read once every minute more or less)
    delay(5000);
