...
void readSensors()
{
  ...
  sprintf(sensorValues, "%s %3.2f %3.2f %3.2f %3.2f %3.2f %3.2f %3.2f %3.2f %3.2f %3.2f", date_time.c_str(), tempValue, tempValueStDev, humValue, humValueStDev, coValue, coValueStDev, no2Value, no2ValueStDev, rsCOValue, rsNO2Value);
...
void writeSDCard(fs::FS &fs, const char *path, const char *message)
{
  ...
  boolean write_head = false;
  char head[] = "date - time - tempValue - tempValueStDev - humValue - humValueStDev - coValue - coValueStDev - no2Value - no2ValueStDev - rsCOValue - rsNO2Value";

  Serial.printf("Writing file: %s\n", path);

  if (!SD.exists(path))
  {
    Serial.println("The file doesn't exist, so we have to write the head");
    write_head = true;
  }
  ...
  // In case the file is just created, we write the head

  if (write_head)
  {
    if (file.println(head))
    {
      Serial.println("Head written in file");
    }
    else
    {
      Serial.println("Head write failed");
    }
  }
